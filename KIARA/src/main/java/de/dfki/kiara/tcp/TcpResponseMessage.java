 ...
 */
public class TcpResponseMessage extends TransportMessage {

    private final HttpHeaders headers;
    private final HttpResponse response;
    private final HttpContent content;

    TcpResponseMessage(TransportConnection connection, HttpHeaders headers) {
        super(connection);
        if (headers == null) {
    ...
    TcpResponseMessage(TransportConnection connection, HttpResponse response, HttpContent content) {
        super(connection);
        if (response == null) {
    ...
    TcpResponseMessage(TransportConnection connection, FullHttpResponse response) {
        super(connection);
        if (response == null) {
            throw new NullPointerException("response");
        }
